...
import { APIRoutes, AppRoutes, CityData, CityName } from '../consts/consts.ts';
...
>('data/fetchSingleOffer', async ({ offerId }, { dispatch, extra: api }) => {
  const { data } = await api.get<OfferData>(`${APIRoutes.Offers}/${offerId}`);
...
>(
  ...
  async ({ offerId, isFavoriteNow }, { dispatch, extra: api }) => {
    const { data } = await api.post<Offer & OfferData>(
...
>('user/login', async (payload, { dispatch, extra: api }) => {
  const { data } = await api.post<UserData>(APIRoutes.Login, payload);
